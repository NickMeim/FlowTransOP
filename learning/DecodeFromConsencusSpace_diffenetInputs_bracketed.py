...
parser = argparse.ArgumentParser(
    description="Run cross-fold validation for comparing with AutoTransOP using bracketed inputs."
)
...
parser.add_argument(
    "--checkpoint_dir",
    type=str,
    default="./chkpts_decoder_diffenetInputs_bracketed",
    help="Directory to store checkpoints and progress.json",
)
parser.add_argument(
    "--log_file",
    type=str,
    default="logs/DecodeFromConsensus_diffenetInputs_bracketed.log",
...
parser.add_argument(
    ...
    default="../preprocessing/preprocessed_data/CellPairs/cmap_all_genes_q1_tas03.csv",
)
parser.add_argument(
    "--output_dir",
    type=str,
    help="Directory to save output results.",
    default="../results/Decoders_only_diffenetInputs_bracketed/",
)

# Bracketed gene sets (copied style from AutoTransOP_Pretrain_FlowMatch_differentInputs_bracketed.py)
parser.add_argument(
    "--brackets",
    metavar="N",
    type=str,
    nargs="*",
    help="Paths for the easier and harder brackets of different gene expression input.",
    default=[
        "../preprocessing/preprocessed_data/SameCellimputationModel/bracketed_difficulty/best_plus_same_features/high_correlation_set_1_iter0.npy",
...
parser.add_argument(
    ...
    type=float,
    default=0.0,
    help="Dropout rate for the input of the decoder.",
)
parser.add_argument(
    "--decoder_activation",
    type=str,
...
def save_progress(path: str, cell_idx: int, fold_id: int, iteration_id: int):
    """Save last COMPLETED (cell_idx, iteration_id, fold_id) triple."""
    state = {
        "last_completed": {
            "cell_idx": cell_idx,
            "fold_id": fold_id,
...
# ----------------------------------------------------------------------
# Main loops over cell lines / brackets / CV folds
# ----------------------------------------------------------------------
for ci, cell in enumerate(cell_lines):
    ...
    recon_csv_path = os.path.join(
        output_dir,
        f"{cell}_Decoder_TransActPaired_reconstruction_differentInputs_bracketed.csv",
    )
    trans_csv_path = os.path.join(
        output_dir,
        f"{cell}_Decoder_TransActPaired_translation_differentInputs_bracketed.csv",
    )
    df_recon_all = _load_or_empty_csv(recon_csv_path)
    df_trans_all = _load_or_empty_csv(trans_csv_path)

    # Loop over brackets instead of random_iterations
    for j, bracket_path in enumerate(brackets):
        if (ci == resume_ptr["cell_idx"]) and (j < resume_ptr["iteration_id"]):
            continue

        # Load genes_1 / genes_2 using same convention as AutoTransOP_Pretrain_FlowMatch_differentInputs_bracketed.py
        if "high_correlation" in bracket_path:
            genes_1 = np.load(bracket_path, allow_pickle=True)
            genes_2 = np.load(
                bracket_path.replace("set_1", "set_2"), allow_pickle=True
            )
        else:
        ...
        for fold_id in range(args.no_folds):
            ...
            print2log(
                f"=== Start fold {fold_id} in iteration {j}/{len(brackets)} for cell {cell} ==="
            )
            ...
            X_2_val_aligned = (
                transact_transform_gpu(
                    X_2_val, initial_alignment_model, space="source"
                )
                .detach()
                .cpu()
                .numpy()
            ...
            X_1_val_aligned = (
                transact_transform_gpu(
                    X_1_val, initial_alignment_model, space="target"
                )
                .detach()
                .cpu()
                .numpy()
            ...
            print2log(
                f"[Saved] {cell} fold={fold_id} iteration={j}/{len(brackets)} "
            ...
            save_progress(progress_path, cell_idx=ci, fold_id=fold_id, iteration_id=j)
            print2log(
                f"=== END fold {fold_id} in iteration {j}/{len(brackets)} for {cell} ==="
            )
