...
for folder in folders:
    ...
    for fold_id in range(1, 6):
        ...
        ## Align the samples in the space using only the training data
        _, _, _, intial_alignment_model = transact_align(
            X_2[pairs_train,:].detach().cpu().numpy(),        # source → will become Z_source
            X_1[pairs_train,:].detach().cpu().numpy(),        # target → will become Z_target
            n_src_pcs=75,
            n_tgt_pcs=75,
            n_pv=30,
            kernel='rbf',
        ...
        df_result.to_csv(output_dir+folder+'_Decoder_TransActPaired_reconstruction_eval_testytest.csv')
        ...
        df_result_translation.to_csv(output_dir+folder+'_Decoder_TransActPaired_translation_eval_testytest.csv')
