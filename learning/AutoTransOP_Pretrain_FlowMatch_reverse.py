...
parser = argparse.ArgumentParser(
    description="Run reverse-direction shared-feature FlowTransOP validation."
)
...
parser.add_argument('--distance_reg',type=float,default=10.,help='Regularization strength for mixing points in the basal latent space')
# parser.add_argument('--lambda_mi_loss', type=float, default=100, help='Weight for the mutual information loss.')
# parser.add_argument('--cosine_loss', type=float, default=10, help='Weight for the cosine loss.')
...
for folder in folders:
    ...
    for fold_id in range(1, 6):
        ...
        # Train only the reverse flow: dataset2 -> dataset1.
        (pearson_2_to_1,cosine_train,flow_21) = train_flowMatch_fold(model_params, device,
                                                                     X_1,X_2,
                                                                     Z_1, Z_2,
                                                                     decoder_1, decoder_2,
                                                                     flow_21,
        ...
        r_2_to_1,pearson1,pearson2,cosine = validate_flowMatch_fold(device, X_1_val, X_2_val, decoder_1, decoder_2, encoder_1, encoder_2,flow_21,pairs_val,'2 to 1')
        # valF1.append(f1)
        # valAcc.append(class_acc)
        valCosine.append(cosine)
        mu_r = 0.5*(np.nanmean(pearson1) + np.nanmean(pearson2))
        mu_r_translation = np.nanmean(r_2_to_1)
        ...
        df_result_2_translation.to_csv(output_dir+folder+'_flow21_TransAct_GeneralizedTransOP_translation_latent_dim30_eval.csv')
        ...
        df_result_latent.to_csv(output_dir+folder+'_flow21_TransAct_GeneralizedTransOP_latent_space_latent_dim30_eval.csv')
