...
parser.add_argument('--checkpoint_dir', type=str, default='./chkpts_training_diffenetInputs',
                    help='Directory to store checkpoints and progress.json')
parser.add_argument('--log_file',type=str,default='logs/AutoTransOP_Pretrain_FlowMatch_differentInputs.log',help='Location of the log file that I can see print outputs')
...
def save_progress(path: str, cell_idx: int, fold_id: int,iteration_idx:int):
    state = {'last_completed': {'cell_idx': cell_idx, 'fold_id': fold_id,'iteration_id':iteration_idx}}
    tmp = path + '.tmp'
...
for ci, cell in enumerate(cell_lines):
    ...
    recon_csv_path = os.path.join(output_dir, f"{cell}_flow_TransAct_GeneralizedTransOP_reconstruction_higherLR_moreReg_differentInputs.csv")
    trans_csv_path  = os.path.join(output_dir, f"{cell}_flow_TransAct_GeneralizedTransOP_translation_higherLR_moreReg_differentInputs.csv")
    ...
    for j in range(random_iterations):
        ...
        for fold_id in range(model_params['no_folds']):
            ...
            print2log(f"=== Start fold {fold_id} in iteration {j}/{random_iterations} for cell {cell} ===")
            ...
            print2log(f'Iteration {j}: Fold {fold_id}: {r1}, {r2}')
            ...
            print2log(f'Train flow for Iteration {j}: Fold {fold_id}: {cell}...')
            ...
            print2log(f'Now train opposite flow for Iteration {j}: Fold {fold_id}: {cell}...')
            ...
            print2log(f'iteration {j}/{random_iterations}: Fold {fold_id}: r1 = {np.nanmean(pearson1):.4f}, r2 = {np.nanmean(pearson2):.4f}')
            print2log(f'iteration {j}/{random_iterations}: Fold {fold_id}: r1_2 = {np.nanmean(r_1_to_2):.4f}, r2_1 = {np.nanmean(r_2_to_1):.4f}')

            # Train shuffled model for the current fold
            flow_12 = Flow(model_params['latent_dim'], int(model_params['latent_dim']/2)).to(device)
            flow_21 = Flow(model_params['latent_dim'], int(model_params['latent_dim']/2)).to(device)
            print2log(f'Train shuffled X flow for iteration {j}/{random_iterations}: Fold {fold_id}: {cell}...')
            ...
            r_1_to_2_shuffledX, _, _,_ = validate_flowMatch_fold(device, X_1_val, X_2_val, decoder_1, decoder_2, encoder_1, encoder_2,flow_12,pairs_val,'1 to 2')
            ...
            print2log(f'[Saved] {cell} fold={fold_id} iretation={j}/{random_iterations} reconstruction r (mean) '
                      f'train={np.nanmean(0.5*(pearson1+pearson2)):.4f} '
                      f'test={np.nanmean(0.5*(r1+r2)):.4f} ')
            print2log(f'[Saved] {cell} fold={fold_id} iretation={j}/{random_iterations} translation r (mean) '
            ...
            print2log(f"=== END fold {fold_id} in iteration {j}/{random_iterations} for {cell} ===")
